...
contract Subscription is Ownable{
  ...
  function getSubscriptionHash(
    ...
    uint tokenAmount, //the token amount paid to the publisher
    ...
    returns (bool)
  {
    ...
    return (signer==from && block.timestamp>=nextValidTimestamp[subscriptionHash]);
  }
  ...
  returns (
    ...
    if(gasPrice>0){
      if(gasToken==address(0)){
        ...
        require(publisherSigned[subscriptionHash],"Publisher has not signed this subscriptionHash");
        require((ERC20(gasToken)).transferFrom(to,msg.sender,gasPrice));
        ...
        require((ERC20(gasToken)).transferFrom(from,to,gasPrice));
      }else{
